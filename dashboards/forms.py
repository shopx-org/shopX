...
class ChangePhoneNumberForm(forms.Form):
    ...
    def clean_new_phone(self):
        phone = strip_tags(self.cleaned_data['new_phone']).strip()

        if phone != self.cleaned_data['new_phone']:
            raise forms.ValidationError(FORBIDDEN_CONTENT_MESSAGE)
...
#     def clean_display_name(self):
#         return strip_tags(self.cleaned_data.get('display_name', '')).strip()
...
#         label="نام نمایشی",
#         required=False,
#         widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'نامی که در سایت نمایش داده شود', 'name': 'display_name'})
#     )
#     national_id = forms.CharField(
#         label="کد ملی",
#         required=False,
...
#         label="تکرار رمز جدید",
#         required=False,
#         widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': 'تکرار رمز جدید', 'name': 'confirm_password'})
#     )
